...
```
The bias of NEBULA-LN in estimating the cell-level overdispersion gets larger when the cell counts per subject becomes lower or the gene expression is more sparse. In contrast, NEBULA-HL is slower, but its accuracy of estimating the overdispersions depends less on these factors. NEBULA-HL will underestimate the subject-level overdispersion if the gene expression is very low. Filtering out low-expressed genes (e.g., counts per cell<0.5%) can be specified by ```cpc=0.005```. A user can select these methods through```method='LN'``` or ```method='HL'```.
