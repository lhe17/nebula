...
The R package, *nebula*, providing fast algorithms for fitting negative
...
large numbers, and NEBULA-HL uses an h-likelihood. NEBULA-LN is faster
...
larger when the cell counts per subject becomes lower or the gene
...
(e.g., counts per cell\<0.5%) can be specified by `cpc=0.005`. A user
can select these methods through`method='LN'` or `method='HL'`.
...
default value is 200, which can provide a good estimate of the
