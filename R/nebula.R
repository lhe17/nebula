...
#'
#' @name nebula
#' @title Association analysis of a multi-subject single-cell data set using a fast negative binomial mixed model
#' @description Performs differential expression analysis using a fast negative binomial mixed model (NEBULA) for multi-subject single-cell data. Includes subject-level random effects to account for hierarchical structure.
#'
                   ...
                   verbose = TRUE, cpc = 0.005, mincp = 5, covariance = FALSE, output_re = FALSE, reml = 0, ncore = 2, fmaxsize = Inf)
{
  ...
  with(plan(multisession, workers = parallelly::availableCores(max = ncore)), local = TRUE)
